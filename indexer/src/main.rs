...
pub struct Args {
    /// Chain to index
    #[arg(short, long, value_enum, default_value_t = Chain::Mainnet)]
    pub chain: Chain,
    /// Database path
    #[arg(short, long)]
...
mod mainnet;
use mainnet::MainnetIndexer;
mod testnet;
use testnet::TestnetIndexer;
mod pallets;
...
async fn main() {
    ...
    match args.chain {
        Chain::Mainnet => {
            acuity_index_substrate::start::<MainnetIndexer>(
                args.db_path,
                args.db_mode.into(),
                db_cache_capacity,
                args.url,
        ...
        Chain::Testnet => {
            acuity_index_substrate::start::<TestnetIndexer>(
                args.db_path,
                args.db_mode.into(),
                db_cache_capacity,
                args.url,
