...
use crate::{ChainKey, MainnetIndexer};

#[tokio::test]
async fn test_process_msg_auction_index() {
    let db_config = sled::Config::new().temporary(true);
    let trees = open_trees::<MainnetIndexer>(db_config).unwrap();
    let indexer = Indexer::<MainnetIndexer>::new_test(trees.clone());
    ...
    let response = process_msg_get_events::<MainnetIndexer>(&trees, key.clone());
